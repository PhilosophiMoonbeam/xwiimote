...
fn every_interface_key_mapping_and_rejected_codes() {
    ...
    let nunchuk = [(BTN_C, BUTTON_C), (BTN_Z, BUTTON_Z)];
    ...
    let drums = [(BTN_START, BUTTON_PLUS), (BTN_SELECT, BUTTON_MINUS)];
...
fn all_absolute_interfaces_update_their_complete_cache() {
    ...
    for &(interface, axes) in cases {
        ...
        assert_eq!(
            ...
            match interface {
                InterfaceKind::Accel => EVENT_CODE_ACCEL,
                InterfaceKind::Ir => EVENT_CODE_IR,
                InterfaceKind::MotionPlus => EVENT_CODE_MOTION_PLUS,
                InterfaceKind::Nunchuk => EVENT_CODE_NUNCHUK_MOVE,
                InterfaceKind::Classic => EVENT_CODE_CLASSIC_CONTROLLER_MOVE,
                InterfaceKind::BalanceBoard => EVENT_CODE_BALANCE_BOARD,
                InterfaceKind::Pro => EVENT_CODE_PRO_CONTROLLER_MOVE,
                InterfaceKind::Drums => EVENT_CODE_DRUMS_MOVE,
                InterfaceKind::Guitar => EVENT_CODE_GUITAR_MOVE,
...
fn ir_starts_with_exact_invalid_sentinels_and_accepts_zero() {
    ...
    for point in d.cache.ir {
        assert!(!is_valid_ir_point(&point));
    }
    assert!(!is_valid_ir_point(&Axis3 {
        x: 1023,
        y: 1023,
        z: 42
    }));
